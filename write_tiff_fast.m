...
% updated Sep. 7, 2020 by Mai-Anh Vu to solve the problem of duplicate
% frames at memory chunk boundaries
...
ip.addParameter('calcmult',3);
ip.addParameter('mode','auto');
...
if isempty(tiff_fields)% Guess based on data & defaults
    ...
    tiff_fields.BitsPerSample = struct('sze',3,'n',1,'value',max(8*ceil(nextpow2(max(vect(data(1,1))))/8),8));
...
maxbuff = min(2^(nextpow2(maxbuff/calcmult/8/tiff_fields.ImageWidth.value/tiff_fields.ImageLength.value)-1),end_ind-start_ind+1);

minn = inf;
maxx = -inf;
last_ind = 0; % added by Mai-Anh Vu, 9/7/2020 - because we kept getting duplicate frames at the buffer ends
for i=1:ceil((end_ind-start_ind+1)/maxbuff)% For each buffer
    %     keyboard
...
%     pause(0.1);
    %ii1 = (i-1)*maxbuff+start_ind;
    ii1 = last_ind+1;
    ii2 = min(i*maxbuff+start_ind,end_ind);
    last_ind = ii2;
    out = permute(data(ii1,ii2),[2 1 3]);
